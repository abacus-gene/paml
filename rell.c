...
int main(int argc, char *argv[])
{
   ...
   if (ntree<=1 || ls<1 || npatt<1) error ("data err");
   if(ntree>NTREE) error("too many trees, raise NTREE?");
   lfh=(double *)malloc(ntree*ls*sizeof(double));
   if(lfh==NULL) error("oom");
   varl=(double *)malloc (ntree*ntree*sizeof(double));
   if(varl==NULL) error("oom");
