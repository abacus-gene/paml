...
struct CommonInfo {
   ...
   int  np, ncode, ntime, nrate, nrgene, nalfa, npi, ncatG, print, MCMC;
   int  priTt,hier, model, clock, given_alfa, given_kapa, given_rgene, Mgene;
   double pi[6], lmax, birth,death,sample, mut, kapa, alfa, beta,delta0,delta1;
...
static double lnpBestTree=-1e40, PMat[16], Cijk[64], Root[4];
...
int main(int argc, char *argv[])
{
   ...
   noisy=3;          
   com.model=F84;    com.kapa=1.63;
   com.alfa=0.;      com.ncatG=1;
...
/*   com.birth=8.2;    com.death=4.1;   com.sample=9./185;   com.mut=0.24; */
   ...
   if (com.alfa) com.fhK=(double*)malloc (com.npatt*com.ncatG*sizeof(double));
   if (com.chunk==NULL || (com.alfa && com.fhK==NULL)) error ("oom");
...
/*
   printf ("la & mu & m\n");
   scanf ("%lf%lf%lf", &com.birth, &com.death, &com.mut);
*/
   SetSeed (1793);
         ...
         for (i=0; i<nopt; i++) {
            if (strncmp(opt, optstr[i], 8)==0)  {
               ...
               switch (i) {
                  ...
                  case ( 6): com.delta0=t;          break;
                  case ( 7): com.delta1=t;          break;
                  case ( 8): com.model=(int)t;      break;
                  case ( 9): com.kapa=(int)t;       break;
                  case (10): com.alfa=(int)t;       break;
                  case (11): com.ncatG=(int)t;      break;
                  case (12): com.hier=(int)t;     break;
                  case (13): com.birth=t;           break;
                  case (14): com.death=t;           break;
                  case (15): com.sample=t;          break;
                  case (16): com.mut=t;             break;
               }
               break;
            }
         }
...
*/
   int i, ir, maxnr=2000000, minnr=200, nrgap=100;
   ...
   for (ir=0,ScaleF=mp=mlnp=sp=slnp=0; ir<maxnr; ir++) {
      ...
      if (com.clock)
         BranchLengthBD (birth, death, com.sample, com.mut);
      else
         FOR (i, tree.nnode) if(i!=tree.origin) nodes[i].branch=rndu()*com.mut;
      lnp=(com.alfa ? -lfundG() : -lfun());
      ...
      if (ir+1>=minnr && (ir+1)%nrgap==0) {
         ...
         d=(d<100?1:1+(int)(d/50));  d=min(d,20);
...
void MCMCunrootedtree (double space[])
{
   ...
   for (irun=0; irun<nrun; irun++)  {
      ...
      OutaTreeN (F0, 0, 0);    
      printf ("%8d %.4f  %9.3f", nchange, lnP, alpha);
...
*/
   struct TREE {struct TREEB tree; struct TREEN nodes[2*NS-1]; } tree0;
   int irun,nrunignore=200,nrun=100000+nrunignore, i,j, nchange;
   int ntreeNNI=2*(com.ns-2-!com.clock), iLH=-1,jLH=0, norder=500;
   int IofLHs[MAXTREEKEPT], counts[MAXTREEKEPT], ntreekept=0, nLHi=-1,nLHj;
   double lnPLHs[MAXTREEKEPT],lnPcur=0,lnP=0,alpha;
   char LH_NNI[(NS-1)*500], TL='T', wantL;
   FILE *ftree, *fLH=NULL;

   for (i=com.ns,j=1; i>2; i--)  j*=i*(i-1)/2;
   printf ("\n%d trees with ordered nodes.\n", j);
 ...
 printf ("%2c %3d %5d %9.3f > %9.3f %8.4f", TL, nLHj,nchange,lnPcur,lnP,alpha);
...
*/
   ...
   fprintf(fout, "\n\nEvaluate LHs (delta=%.3f):\n\n", delta);
   if (fLH) {
      fscanf(fLH, "%d%d", &i, &ntreekept);
      if (i!=com.ns || ntreekept>MAXTREEKEPT)  error ("LHfile err.");
   }
   for (j=0; j<ntreekept; j++) {
