/* listtree.c
   Copyright, Ziheng Yang, April 1995.
     ...
     gcc -O2 -o listtree listtree.c tools.o
     listtree
*/
...
struct CommonInfo {
   ...
   double kapa, alfa, pi[6],*rates, *chunk;
}  com;
struct TREEB {
...
int main (void)
{
   ...
   for (; ;) {
      ...
      puts ("\t\t(5) Calculate identical bi-partitions between trees?\n");
      puts ("\t\t(6) Simulate data sets?\n");
      puts ("\t\t(0) Quit?\n");
      ...
      if (option<6)  { printf ("No. of species: ");   scanf ("%d", &com.ns); }
      if (com.ns>NS) error ("No. of species too large.. raise NS.");

      rooted=!(option%2);
      if (option==6) SimulateData();
      else if (option<3) {
...
int TreeDistances (void)
{
   ...
   printf ("\nThis calculates the number of identical bi-partitions ");
...
*/
   FILE *fseq, *ftree;
   char seqf[32]="simulateddata", treef[32];
   int i,j, ir, nr, fixtree=0, ncatG, size=10000;
   double birth=0, death=0, sample=1, mut=1, *space;

   fseq=fopen(seqf, "w");
   printf ("\nGenerated sequences go into file %s\n", seqf);
   ...
   if (fixtree) {
      ...
      printf ("\nModel tree: ");  OutaTreeN (F0,0,1); FPN(F0);
   }
   else {
   ...
   if (com.z[com.ns-2-1-1]==NULL || space==NULL) error("oom");

   for (ir=0,com.ncode=4,com.clock=1; ir<nr; ir++) {
      if (!fixtree) {
         RandomLHistory (1, space);
         BranchLengthBD (birth, death, sample, mut);
         printf ("\ntree used: ");  OutaTreeN (F0,0,1);  FPN(F0);
      }
      if (com.alfa) 
         Rates4Sites (com.rates,com.alfa,ncatG,com.ls, !com.model,space);
