...
double DistanceREV (double Ft[], int n, double alfa, double Root[], double U[],
   double V[], double pi[], double space[], int *cond);
int PMatBranch (double Ptb[], int n, double branch[], 
    double pi[], double Root[], double U[], double V[], double space[]);
int PatternLS (FILE *fout, double Ft[],double alfa, double space[], int *cond);
int testx (double x[], int np);
int GetOptions (char *ctlf);
int AlfaMP (FILE* fout);
...
struct CommonInfo {
   ...
   int np, ntime, ncode,given_kapa,given_rgene,given_alfa, clock, model, ncatG;
   double lmax, pi[NCODE], kapa, alfa, rou, rgene[NGENE], piG[NGENE][NCODE];
   double *chunk;
}  com;
struct TREEB {
...
int testxAlfaMP (double x[], int nx);
double lfunAlfa_Sullivan (double x[], int nx);
double lfunAlfa_YK (double x[], int nx);
...
int AlfaMP (FILE* fout)
{
   ...
   if (x[0]<=0) x[0]=9;
   ming1 (NULL, &lnL, lfunAlfa_Sullivan, NULL, testxAlfaMP, x, space, 1e-6, 1);
   fprintf (fout, "\nalpha (Sullivan et al. 1995)%9.4f\n", x[0]);

   MuChange/=tree.nbranch; 

   ming1 (NULL, &lnL, lfunAlfa_YK, NULL, testxAlfaMP, x, space, 1e-6, 1);
...
double lfunAlfa_YK (double x[], int nx)
{
   ...
   double lnL=0, prob, alfa=x[0], t=(nx==2?x[1]:MuChange), p;
   double freqK[NCATG], rK[NCATG];

   DiscreteGamma (freqK, rK, alfa, alfa, com.ncatG, 0);
   FOR (k, maxchange+1) {
      if (NSiteChange[k]==0) continue;
...
*/
   ...
   if (com.nhomo==0) 
      PMatBranch (Ft, n, branch, pi, Root, U, V, space);
   else {
...
*/
   ...
   for (h=0; h<com.npatt; h++) {
      ...
      NSiteChange[nchange]+=com.fpatt[h];
...
*/
   FOR (i,n) {
      ...
      Root[i]=(alfa<=0?log(Root[i]):gammap(Root[i],alfa));
   }
   ...
   for (i=0,zero(Qt,n*n),zero(Qm,n*n); i<com.ns; i++) {
      for (j=0; j<i; j++) {
	    ...
	    Q[(com.z[i][h]-1)*n+com.z[j][h]-1] += com.fpatt[h]/2.;
         ...
         it=i*(i-1)/2+j;
	 SeqDistance[it]=DistanceREV (Q, n, alfa, Root,U,V, pi, space, &k);
...
*/
   ...
   fclose (fdist);
   DistanceREV (Qt, n, alfa, Root, U, V, pi, space, &k);
   ...
   if (tree.nbranch) {
      ...
      OutaTreeB (fout);  FPN (fout);
      FOR (i,tree.nbranch) fprintf(fout,"%9.5f", branch[i]);
      PMatBranch (Ft, com.ncode, branch, pi, Root, U, V, space);
   }
   return (0);
}
